...
export default function AttendanceForm({ students, sectionId }: { students: any[], sectionId: string }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch('/api/teacher/attendance', {
                ...
                body: JSON.stringify({ sectionId, records: attendance }),
            });
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-purple-100/50 border border-slate-200 overflow-hidden">
                <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">Marking records for today</p>
                    </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-50">
                                ...
                                <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Live Status Toggle</th>
                            </tr>
                        </thead>
                                    ...
                                    <tr key={student.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="px-10 py-6">
                                            <div className="flex items-center gap-5">
                                                <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-purple-600 group-hover:text-white transition-all overflow-hidden border-2 border-white shadow-sm ring-1 ring-slate-100">
